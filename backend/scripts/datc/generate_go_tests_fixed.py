#!/usr/bin/env python3
"""
Go Test Generator for DATC Test Cases

Generates Go test files from extracted DATC test cases.

Usage:
    python generate_go_tests_fixed.py datc_tests_fixed.json --category basic_checks --output-dir ../../internal/game/datc/
"""
...
"""
    ...
    def generate_subset(
        ...
        """Generate a subset of test cases."""
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate Go test files from DATC test cases"
    )
    parser.add_argument("input_file", help="Path to JSON file with test cases")
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("."),
    ...
    parser.add_argument(
        "--subset", help="Generate subset in format 'start,count' (e.g., '0,5')"
    )
    parser.add_argument(
        "--by-category",
        action="store_true",
        help="Generate separate files for each category",
    )
    ...
    if args.by_category:
        generator.generate_by_category(test_cases, args.output_dir)
    elif args.category:
        # Filter by category
        filtered_cases = [
        ...
        filename = f"datc_{args.category}_test.go"
        output_path = args.output_dir / filename
        generator.generate_test_file(filtered_cases, output_path)
    elif args.subset:
        try:
            start, count = map(int, args.subset.split(","))
            filename = f"datc_subset_{start}_{start + count - 1}_test.go"
            output_path = args.output_dir / filename
            generator.generate_subset(test_cases, start, count, output_path)
        except ValueError:
            print("Invalid subset format. Use 'start,count' (e.g., '0,5')")
            return
    else:
        # Generate all tests in one file
        output_path = args.output_dir / "datc_all_tests.go"
